...
describe("PATCH /api/v1/activations/[token_id]", () => {
  describe("Anonymous user", () => {
    test("With nonexistent token", async () => {
      const response = await fetch(
        `${webserver.origin}/api/v1/activations/${uuidv4()}`,
        {
          method: "PATCH",
        },
      );
      ...
      expect(responseBody).toEqual({
        name: "NotFoundError",
        message: "O token de ativação informado não é válido ou expirou.",
    ...
    test("With valid token", async () => {
      ...
      const activatedUser = await user.getById(activationToken.user_id);
    ...
    test("With valid token but already activated user", async () => {
      ...
      expect(responseBody).toEqual({
        ...
        action:
          'Verifique se você tem a feature "read:activation_token" e tente novamente.',
        status_code: 403,
      });
    });
  });
  ...
  describe("Default user", () => {
    test("With valid token, but already logged in user", async () => {
      ...
      const user1SessionObject = await orchestrator.createSessionObject(user1);
